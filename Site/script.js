(function () {
    ...
    async function fetchCode(action, id, isIL) {
        ...
        var orig = await fetchJsonPOST(`/${action}/${id}/original`).catch(
            console.error,
        );
        ...
        var tr = await fetchJsonPOST(`/${action}/${id}/transpiled`).catch(
            console.error,
        );
        if (!tr || tr.exception) {
        ...
        breath(() => {
            ...
            if (orig) $("#code1 code").textContent = orig.code;

            if (tr) $("#code2 code").textContent = tr.code;
    ...
    function diffAndSetCode(code1, code2, ignoreIlLabels) {
        ...
        var todiff = [code1, code2].map((c) =>
            c
                .split(NON_TRAILING_NEWLINE)
                .map((l) => l.replace(LEADING_TABS, ""))
                .join("\n"),
        );
        if (ignoreIlLabels)
        ...
        for (var i = 0; i < diff.length; i++) {
            ...
            if (part.removed && diff[i + 1] && diff[i + 1].added) {
                ...
                if (d > 0) {
                    added += `${p + 1}-${short},${short + 1}-${
                        short + Math.abs(d)
                    },`;
                    removed += `${p + 1}-${curline},`;
                } else {
                    added += `${p + 1}-${curline},`;
                    removed += `${p + 1}-${short},${short + 1}-${
                        short + Math.abs(d)
                    },`;
                }
        ...
        for (var i = 0; i < linesLeft.length; i++) {
            ...
            var displayedLinesLeft = Math.round(
                parseFloat(rowsLeft.children[i].style.height) / oneLineHeight,
            );
            var displayedLinesRight = Math.round(
                parseFloat(rowsRight.children[i].style.height) / oneLineHeight,
            );
            ...
            if (displayedLinesLeft > displayedLinesRight) {
                linesRight[i] += "\u200b\n\u200c".repeat(
                    displayedLinesLeft - displayedLinesRight,
                );
            } else if (displayedLinesRight > displayedLinesLeft) {
                linesLeft[i] += "\u200b\n\u200c".repeat(
                    displayedLinesRight - displayedLinesLeft,
                );
            }
        }
    ...
    function onChangeLang(e) {
        ...
        fetchCode(isil ? "disasm" : "decomp", select.value, isil);
    }
    ...
    function changeListDisabled(status) {
        for (var opt of $$("#tr-select option")) opt.disabled = status;
    }
